#!/usr/bin/env python3

ROOMS = {
    'entrance': {
        'description': 'Вы в темном входе лабиринта. Стены покрыты мхом. '
                       'На полу лежит старый факел.',
        'exits': {'north': 'hall', 'east': 'trap_room'},
        'items': ['torch'],
        'puzzle': None
    },
    'hall': {
        'description': 'Большой зал с эхом. По центру стоит пьедестал '
                       'с запечатанным сундуком.',
        'exits': {'south': 'entrance', 'west': 'library', 'north': 'treasure_room'},
        'items': ['coin'],
        'puzzle': (
            'На пьедестале надпись: "Назовите число, которое идет после девяти". '
            'Введите ответ цифрой или словом.',
            '10'
        )
    },
    'trap_room': {
        'description': 'Комната с хитрой плиточной поломкой. На стене видна '
                       'надпись: "Осторожно — ловушка".',
        'exits': {'west': 'entrance', 'east': 'forge'},
        'items': ['rusty_key'],
        'puzzle': (
            'Система плит активна. Чтобы пройти, назовите слово "шаг" '
            'три раза подряд (введите "шаг шаг шаг")',
            'шаг шаг шаг'
        )
    },
    'library': {
        'description': 'Пыльная библиотека. На полках старые свитки. '
                       'Где-то здесь может быть ключ от сокровищницы.',
        'exits': {'east': 'hall', 'north': 'armory', 'south': 'garden'},
        'items': ['ancient_book'],
        'puzzle': (
            'В одном свитке загадка: "Что растет, когда его съедают?" '
            '(ответ одно слово)',
            'резонанс'
        )
    },
    'armory': {
        'description': 'Старая оружейная комната. На стене висит меч, '
                       'рядом — небольшая бронзовая шкатулка.',
        'exits': {'south': 'library'},
        'items': ['sword', 'bronze_box'],
        'puzzle': None
    },
    'treasure_room': {
        'description': 'Комната, на столе большой сундук. Дверь заперта — '
                       'нужен особый ключ.',
        'exits': {'south': 'hall'},
        'items': ['treasure_chest'],
        'puzzle': (
            'Дверь защищена кодом. Введите код (подсказка: это число '
            'пятикратного шага, 2*5= ? )',
            '10'
        )
    },
    'garden': {
        'description': 'Тайный сад с волшебными растениями. В центре '
                       'растет сияющий цветок.',
        'exits': {'north': 'library'},
        'items': ['magic_flower', 'healing_potion'],
        'puzzle': (
            'Цветок говорит: "Я легче пера, но сильнее стали. Что я?"',
            'дыхание'
        )
    },
    'forge': {
        'description': 'Кузница с раскаленными углями. На наковальне '
                       'лежит незаконченный меч.',
        'exits': {'west': 'trap_room'},
        'items': ['hammer', 'unfinished_sword', 'treasure_key'],
        'puzzle': (
            'На стене написано: "Что можно сломать, даже если никогда '
            'не брать в руки?"',
            'обещание'
        )
    }
}

COMMANDS = {
    "осмотреться/look": "посмотреть вокруг",
    "инвентарь/inventory": "посмотреть инвентарь",
    "взять [предмет]/take [item]": "подобрать предмет",
    "использовать [предмет]/use [item]": "использовать предмет",
    "идти [направление]/go [direction]": "переместиться",
    "north/south/east/west": "переместиться в направлении",
    "решить/solve": "решить загадку или открыть сундук",
    "открыть/open": "попытаться открыть сундук с сокровищами",
    "выход/quit": "выйти из игры",
    "помощь/help": "показать эту подсказку"
}
